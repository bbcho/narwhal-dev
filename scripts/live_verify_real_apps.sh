...
log_file="$(mktemp "${TMPDIR:-/tmp}/narwhal-real-app-verify.XXXXXX")"
trap 'rm -f "$log_file"' EXIT

echo "This verifier launches and moves real Firefox, Chrome, and System Settings windows, then restores their frames."
...
if grep -Eq 'recorded an issue|failed after .* with [1-9][0-9]* issue' "$log_file"; then
  echo "live_verify_real_apps failed: verifier output contains failing issues" >&2
  exit 1
fi

if ! grep -Eq 'Test run with [1-9][0-9]* tests? in [1-9][0-9]* suites? passed' "$log_file"; then
  echo "live_verify_real_apps failed: Swift Testing did not report a completed passing test run" >&2
  exit 1
fi

...
enum CommandOverlayVerification {
    static func verifyDefaultTwoColumnLayout() -> (passed: Bool, message: String) {
        ...
        return (
            true,
            "command overlay split verified: viewport=\(snapshot.viewportBounds.debugDescription) document=\(snapshot.documentBounds.debugDescription) left=\(left.debugDescription) separator=\(separator.debugDescription) right=\(right.debugDescription)"
        )
    }
}
...
private enum CommandOverlayCategory: CaseIterable {
    ...
    var purpose: String {
        ...
        case .system:
            return "Reload, inspect, or reset WinMgr state."
        }
    }
}
...
private func commandOverlaySections(
    ...
    return [
        CommandOverlaySection(
            title: "System",
            purpose: "Reload, inspect, or reset WinMgr state.",
            rows: [
                CommandOverlayRow(
                    key: "",
